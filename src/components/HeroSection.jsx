...
const HeroSection = ({ pref }) => {
  ...
  return (
    <motion.div className=" relative flex flex-col md:flex-row md:h-screen  py-11 md:py-0 md:justify-center items-center text-center  overflow-hidden font-bold  text-text">
      ...
      <motion.div className=" w-full h-full  relative flex items-center ">
        <div className="w-2/5 h-full relative group " >
          <img src={blob} className="absolute top-1/3" alt="" />
          <motion.img
            onMouseMove={handleMouseMove} onMouseLeave={() => { setPosition({ x: 0, y: 0 }); setdisp(true) }}
            ref={targetRef}
            className=" w-full h-full relative z-10  object-contain object-center   "
            src={sd}
            alt="my-image"
            loading="lazy"
          />
